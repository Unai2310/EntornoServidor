...
class AccesoDatos {
   ...
   // Constructor privado  Patron singleton
    ...
    private function __construct(){
        ...
        try {
            $dsn = "mysql:host=".SERVER_DB.";dbname=Examen;charset=utf8";
            $this->dbh = new PDO($dsn, "unai", "Un@isql23");
        ...
        $this->stmt_modsal   = $this->dbh->prepare("update Usuarios set saldo=:saldo where login=:login");
    }
    ...
    public function aumentarSaldo($saldo,$login) {
        $this->stmt_modsal->bindValue(':saldo', $saldo);
        $this->stmt_modsal->bindValue(':login',$login);
        $this->stmt_modsal->execute();
        $resu = ($this->stmt_moduser->rowCount () == 1);
        return $resu;
